...
def PolyOver(ring):
    def poly_depth(ring):
        depth = 0
        while issubclass(ring, PolyType):
            ring = ring.ring
            depth += 1
        return depth
    def depth_to_varstr(n):
        assert type(n) == int and n >= 0
        if n <= 5:
            return ["λ", "μ", "ξ", "α", "β", "γ"][n]
        else:
            return f"[var_{n}]"
    varstr = depth_to_varstr(poly_depth(ring))
    
    class Poly(PolyType, basic.Euclidean):
        @classmethod
        def typestr(cls):
            return f"{ring}[{varstr}]"
        @classmethod
        def int(cls, n):
            assert type(n) == int
            return cls([n])
        ...
        def __init__(self, rep):
            ...
            while (False if len(rep) == 0 else rep[-1] == 0):
                rep = rep[:-1]
            self.rep = rep
        ...
        def __str__(self):
            bo = "("
            bc = ")"
            if type(self).ring in [basic.ZZ, basic.QQ]:
                bo = ""
                bc = ""
            ...
            def strterm(c, p):
                ...
                if p == 1:
                    ...
                    var_term_str = f"{varstr}^{p}"
                if c == 0:
                    return ""
                elif str(c) == "1":
                    return "+" + bo + var_term_str + bc
                elif str(c) == "-1":
                    return "-" + bo + var_term_str + bc
                else:
                    return strterm(c, 0) + bo + var_term_str + bc
...
##        def __repr__(self):
##            return f"{type(self).typestr()}({', '.join(repr(x) for x in self.rep)})"
        def __repr__(self):
